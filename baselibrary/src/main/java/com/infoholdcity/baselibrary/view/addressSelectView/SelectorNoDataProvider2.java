...
public class SelectorNoDataProvider2 implements AdapterView.OnItemClickListener {
    ...
    public SelectorNoDataProvider2(Context context, Area initArea, int deep) {
            ...
            if (subArea != null && subArea.size() > 0) {
                if (subArea.size() == 1) {//最后一级有的为“ ”过滤掉
                    if (subArea.get(0).getName().equals(" ")) {
                        ...
                        setDataToAddressList(value, parentName, subArea, addressData);
                    }
                } else {
                    setDataToAddressList(value, parentName, subArea, addressData);
                }
    ...
    private void setDataToAddressList(String value, String parentName, List<Area> subAreas, ArrayList<ISelectAble> addressList) {
